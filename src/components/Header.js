...
function Header(props) {
	...
	const toggleDrawer = (drawerState) => setState(state => ({...state, showDrawer: drawerState}))
	return (
		<div id='header'>
			<div id="header-info">
				...
				{props.user && <img style={{height: '50px', width: '50px', margin: '0 5px', borderRadius: '50%'}} src={props.user.image || profilePlaceholder} alt={props.user.username} />}
			...
			<div id='header-options'>
				{props.inDrawer && <Link to={`/user/${props.user.username}`}><RiSettingsFill /></Link>}
			</div>
		</div>
	)
}
